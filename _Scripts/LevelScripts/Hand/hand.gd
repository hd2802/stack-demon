...
func add_card(card_id : String):
	...
	tween.tween_property(new_card, "scale", Vector2(1, 1), 0.4).set_trans(Tween.TRANS_BOUNCE)
...
func _on_play_card_button_pressed() -> void:
	if !selected_card:
		...
		var tween = get_tree().create_tween()
		tween.tween_property(selected_card, "position", selected_card.position + Vector2(0, -200), 0.4).set_trans(Tween.TRANS_CUBIC)
